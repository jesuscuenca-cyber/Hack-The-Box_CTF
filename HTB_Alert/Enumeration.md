...
```bash
$ sudo nmap -p- --open -sS -sC -sV --min-rate 5000 -n -Pn -A 10.10.10.44 -oX tcp_scan.xml
...
First of all we will try to open with our browser the IP. To access the website, we must add the domain name alert.htb to our /etc/hosts file to resolve the connection with the IP address. In our Nmap scan we can see the url http://alert.htb/ so we add in our directory.
```bash
...
In both cases we can see the apache version is runing in the system. Now we can try fuzzing to see all the directories contained in the displayed web page. We also see that there is an option to upload some type of file.
