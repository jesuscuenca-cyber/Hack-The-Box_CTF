...
```bash
$ sudo nmap -p- --open -sS -sC -sV --min-rate 5000 -n -Pn -A 10.129.17.166 -oG tcp_scan.xml
```
...
```bash
$ echo "10.129.17.166  Shocker.htb " | sudo tee -a /etc/hosts 
```

![alt text](Images/Hosts.png)

Now we can go to the IP with our browser. 
