...
```bash
$ sudo nmap -p- --open -sS -sC -sV --min-rate 5000 -n -Pn -A 10.10.10.68 -oX tcp_scan.xml
...
```bash
$ echo "10.10.10.68" | sudo tee -a /etc/hosts
```
...
In both cases we can see the apache version is runing in the system. They do not show us any interesting data.
